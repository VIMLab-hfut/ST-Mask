...
def PredictImg( image, model, device):
    ...
    result = img.copy()
    mask_11 = np.zeros((img.shape[0], img.shape[1], 3), dtype=np.int)
    dst = img.copy()
    img = toTensor(img)

    names = {'0': 'background', '1': 'ST'}
    ...
    for idx in range(boxes.shape[0]):
        if scores[idx] >= 0.1:#阈值
            m_bOK = True
            color=random_color()
            mask=masks[idx, 0].mul(255).byte().cpu().numpy()
            mask_11 = masks[idx, 0].mul(255).byte().cpu().numpy()
            thresh = mask
...
if __name__ == "__main__":
    ...
    model.to(device)
    model.eval()
    save = torch.load('pth/model_test_100_update.pth')
    model.load_state_dict(save['model'])
    start_time = time.time()#返回当前时间
    print(start_time)
    image = PredictImg('1_3_4.jpg', model, device)
    print(2)
